...
| **质疑** ×1（清算） | 只在「他的话有破绽」时亮起 | 追打已暴露但错过的破绽（= 抓千窗口的事后补刀） |

**抓千（破绽窗口）**：三拍 —— **①他下注（注码可见）②他说话（狠话可见）③两样对不上**
→ 气泡与注码同时发亮、心跳响起，倒计时内按下「抓千！」→ Hitstop 打击演出 +
...
| 😳 动摇 | 更凶，你不能当他好欺负 | 漏话最多 → 抓千窗口变多、READ 更清晰 |
...
| `transitions` | 心理事件 → 状态转移表，**双向**：打击向下（`BLUFF_CAUGHT`、`CONTRADICTION_EXPOSED`…含从正轨打断），赢钱向上（`BIG_POT_WON`、`ALL_IN_WON`、`WIN_STREAK`、`HAND_LOST` 掉档）；抓千/言语永不回血 |
...
│   ├── battle.js           战斗编排：手牌流转、台词/破绽/抓千、READ/言语、胜负心理事件、结算
│   ├── balance.json        ★ 全部可调数值
...
检测到矛盾 → 播台词 → 开破绽窗口（三拍演出）。**抓千点击成功 = 窗口必然有效**
...
| POST | `/api/object` | `{id}` 抓千 → `{view, events, ok, reason?}` |
...
- **战斗**（17）：视图隐私扫描、门禁与次数、抓千命中/过期/重复点击、质疑落空与命中、
